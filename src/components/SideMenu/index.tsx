...
class SideMenu extends React.Component<ISideMenuProps, State> {
  ...
  getNavMenuItems = (menuData: MenuDataItem[], isSubMenu = false): Array<React.ReactElement> => {
    ...
    const { menuMarkMap } = this.props.permission
  ...
  filterMenuData = (menuData: MenuDataItem[], permissionMap: PermissionModelState['menuMarkMap']): MenuDataItem[]=>{
    function dfs(menuList: MenuDataItem[], ret=[]): MenuDataItem[]{
      for ( let i=0; i<menuList.length; i++ ) {
        let item = menuList[i]
        if (!item.permission || permissionMap[item.permission]) {
          if (item.children) {
            let children = dfs(item.children)
    ...
    return (
      ...
      >
        ...
        </div>
        {
          ...
          >
            {this.getNavMenuItems(this.filterMenuData(this.props.menuData, this.props.permission.menuMarkMap))}
          </Menu>
        }
